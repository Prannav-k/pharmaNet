...
const { validatemanufacturerInitiator } = require('./lib/utils');
...
class manufacturerContract extends Contract {
  ...
  async approveNewUser(ctx, name, aadharId) {
    ...
    // with new field upgrad coins
    const userObj = User.createInstance(Object.assign(userReqObj, { upgradCoins: 0, createdAt: new Date() }));
    await ctx.userList
      .addUser(userObj)
      .then();
   ...
   * @returns {Object}
   */
  async approvePropertypharma(ctx, propertyId) {
    validatemanufacturerInitiator(ctx);
